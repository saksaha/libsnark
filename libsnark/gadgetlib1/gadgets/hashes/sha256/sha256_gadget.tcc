...
sha256_compression_function_gadget<FieldT>::sha256_compression_function_gadget(
  ...
  for (size_t i = 0; i < 64; ++i) {
    ...
    round_functions.push_back(sha256_round_function_gadget<FieldT>(
        pb, round_a[i], round_b[i], round_c[i], round_d[i], round_e[i],
        round_f[i], round_g[i], round_h[i], packed_W[i], SHA256_K[i],
  ...
  unreduced_output.allocate(pb, 8,
                            FMT(this->annotation_prefix, " unreduced_output"));
  reduced_output.allocate(pb, 8,
                          FMT(this->annotation_prefix, " reduced_output"));
  for (size_t i = 0; i < 8; ++i) {
    reduce_output.push_back(lastbits_gadget<FieldT>(
        pb, unreduced_output[i], 32 + 1, reduced_output[i],
        pb_variable_array<FieldT>(output.bits.rbegin() + (7 - i) * 32,
...
sha256_two_to_one_hash_gadget<FieldT>::sha256_two_to_one_hash_gadget(
    protoboard<FieldT> &pb, const digest_variable<FieldT> &left,
  ...
  f.reset(new sha256_compression_function_gadget<FieldT>(
      pb, SHA256_default_IV<FieldT>(pb), block, output,
      FMT(this->annotation_prefix, " f")));
}

template <typename FieldT>
sha256_two_to_one_hash_gadget<FieldT>::sha256_two_to_one_hash_gadget(
    protoboard<FieldT> &pb, const size_t block_length,
...
#endif
  f.reset(new sha256_compression_function_gadget<FieldT>(
      pb, SHA256_default_IV<FieldT>(pb), input_block.bits, output,
      FMT(this->annotation_prefix, " f")));
}
...
libff::bit_vector sha256_two_to_one_hash_gadget<FieldT>::get_hash(
  ...
  sha256_two_to_one_hash_gadget<FieldT> f(pb, SHA256_block_size, input_variable,
                                          output_variable, "f");
